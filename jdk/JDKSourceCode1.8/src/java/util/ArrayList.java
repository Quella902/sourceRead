 ...
 * 容量可以自动增长。
 * 如果提前知道数组元素较多，可以在添加元素前通过调用ensureCapacity()方法提前增加容量以减小后期容
 * 量自动增长的开销。
 ...
 * 子类是否使用此字段是可选的。
 * 如果子类希望提供快速失败迭代器（和列表迭代器），则它只需在其 add(int,e)和remove(int)方法（以及它所重写的、
 * 导致列表结构上修改的任何其他方法）中增加此字段。
...
public class ArrayList<E> extends AbstractList<E>
    ...
    /**
     * 带有容量initialCapacity  /ɪˈnɪʃl/   /kəˈpæsəti/ 的构造方法
     *
     * @param 初始容量列表的初始容量
    ...
    public ArrayList(Collection<? extends E> c) {
        ...
        if ((size = elementData.length) != 0) {
            ...
            // 编译不会报错 具体c.toArray()返回的是不是Object[] 不一定
